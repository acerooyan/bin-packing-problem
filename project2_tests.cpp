...
void testAlgorithm(ProblemInstance test, ProblemInstance expected_result, algorithm algo, std::string name) {
    ...
    if (test.assignments == expected_result.assignments and compare(test.free_space, expected_result.free_space)) {
        ...
        //std::cout << "\n failed test ass\n";
        for(int i =0; i < test.assignments.size(); i++){

            //std::cout << i << ":" << test.assignments[i] << '\n';
        }

        //std::cout << "\n  ass\n";
        for(int i =0; i < expected_result.assignments.size(); i++){

            //std::cout << i << ":" << expected_result.assignments[i] << '\n';
        }
...
int main() {
    ...
    std::vector<InsertType<int, char>> data = {{4, 'a', 0},
                                               {5, 'b', 0},
                                               {2, 'c', 2},
                                               {1, 'd', 1}};
    ...
    std::vector<InsertType<int, char>> data2 = {{4, 'a', 2},
                                                {5, 'b', 3},
                                                {2, 'c', 1},
                                                {1, 'd', 0},
                                                {0, 'e', 1}};
    ...
    for (int i = 0; i < max_value; ++i) {
        tree3.insert(my_vector[i], my_vector[i]);
    }

    std::random_shuffle(my_vector.begin(), my_vector.end());
    for (int i = 0; i < max_value; ++i) {
        tree3.remove(my_vector[i]);
    }


    for (int i = 100; i < max_value; i++) {
        printf("%d \n", tree3.find(my_vector[i]));
    }
    printf("finished \n");
    ...
    items = {0.54, 0.67, 0.46, 0.57, 0.06, 0.23, 0.83, 0.64, 0.47, 0.03, 0.53, 0.74, 0.36, 0.24, 0.07, 0.25, 0.05, 0.63,
             0.43, 0.04};
    assignments = std::vector<int>(items.size(), 0);
    ProblemInstance test4 = {items, assignments, free_space};

    // next_fit
    expected_result = {items, {1, 2, 3, 4, 4, 4, 5, 6, 7, 7, 8, 9, 10, 10, 10, 10, 10, 11, 12, 12},
                       {0.46, 0.33, 0.54, 0.14, 0.17, 0.36, 0.5, 0.47, 0.26, 0.03, 0.37, 0.53}};
    testAlgorithm(test4, expected_result, next_fit, "next_fit");

    // first_fit
    expected_result = {items, {1, 2, 1, 3, 2, 2, 4, 5, 6, 2, 6, 7, 3, 5, 3, 7, 4, 8, 9, 4},
                       {0, 0.01, 0, 0.08, 0.12, 0, 0.01, 0.37, 0.57}};
    testAlgorithm(test4, expected_result, first_fit, "first_fit");

    // first_fit_decreasing
    expected_result = {items, {1, 2, 3, 4, 5, 6, 7, 8, 8, 7, 6, 4, 2, 3, 5, 1, 1, 3, 1, 3},
                       {0, 0.01, 0.01, 0, 0.14, 0, 0, 0}};
    testAlgorithm(test4, expected_result, first_fit_decreasing, "first_fit_decreasing");

    // best_fit

    expected_result = {items, {1, 2, 1, 3, 2, 2, 4, 5, 6, 2, 6, 7, 5, 7, 4, 3, 4, 8, 9, 4},
                       {0, 0.01, 0.18, 0.01, 0, 0, 0.02, 0.37, 0.57}};
    testAlgorithm(test4, expected_result, best_fit, "best_fit");

    // best_fit_decreasing
    expected_result = {items, {1, 2, 3, 4, 5, 6, 7, 8, 8, 7, 6, 4, 2, 3, 5, 3, 5, 5, 1, 5},
                       {0.13, 0.01, 0.02, 0, 0, 0, 0, 0}};
    testAlgorithm(test4, expected_result, best_fit_decreasing, "best_fit_decreasing");

    // ----------------------------------test 5----------------------------------
    std::cout << std::endl;
    std::cout << "test 5" << std::endl;
    items = {0.5, 0.6, 0.5, 0.3, 0.4, 0.1, 0.5, 0.6};
    assignments = std::vector<int>(items.size(), 0);
    ProblemInstance test5 = {items, assignments, free_space};
    // next_fit
    expected_result = {items, {1, 2, 3, 3, 4, 4, 4, 5}, {0.5, 0.4, 0.2, 0, 0.4}};
    testAlgorithm(test5, expected_result, next_fit, "next_fit");

    // first_fit
    expected_result = {items, {1, 2, 1, 2, 3, 2, 3, 4}, {0.0, 0.0, 0.1, 0.4}};
    testAlgorithm(test5, expected_result, first_fit, "first_fit");

    // first_fit_decreasing
    expected_result = {items, {1, 2, 3, 3, 4, 1, 2, 2}, {0.0, 0.0, 0.0, 0.5}};
    testAlgorithm(test5, expected_result, first_fit_decreasing, "first_fit_decreasing");

    // best_fit
    expected_result = {items, {1, 2, 1, 2, 3, 2, 3, 4}, {0.0, 0.0, 0.1, 0.4}};
    testAlgorithm(test5, expected_result, best_fit, "best_fit");

    // best_fit_decreasing
    expected_result = {items, {1, 2, 3, 3, 4, 1, 2, 2}, {0.0, 0.0, 0.0, 0.5}};
    testAlgorithm(test5, expected_result, best_fit_decreasing, "best_fit_decreasing");


    // ----------------------------------test 6----------------------------------
    std::cout << std::endl;
    std::cout << "test 6" << std::endl;
    items = {.8, .9, .8, .9, .8, .2, .2, .2};
    assignments = std::vector<int>(items.size(), 0);
    ProblemInstance test6 = {items, assignments, free_space};
    // next_fit
    expected_result = {items, {1, 2, 3, 4, 5, 5, 6, 6}, {0.2, 0.1, 0.2, 0.1, 0.0, 0.6}};
    testAlgorithm(test6, expected_result, next_fit, "next_fit");

    // first_fit
    expected_result = {items, {1, 2, 3, 4, 5, 1, 3, 5}, {0.0, 0.1, 0.0, 0.1, 0.0}};
    testAlgorithm(test6, expected_result, first_fit, "first_fit");

    // first_fit_decreasing
    expected_result = {items, {1, 2, 3, 4, 5, 3, 4, 5}, {0.1, 0.1, 0, 0, 0}};
    testAlgorithm(test6, expected_result, first_fit_decreasing, "first_fit_decreasing");

    // best_fit_decreasing
    expected_result = {items, {1, 2, 3, 4, 5, 3, 4,    5}, {0.1, 0.1, 0, 0, 0}};
    testAlgorithm(test6, expected_result, best_fit_decreasing, "best_fit_decreasing");


    // best_fit
    expected_result = {items, {1, 2, 3, 4, 5, 1, 3, 5}, {0.0, 0.1, 0.0, 0.1, 0.0}};
    //testAlgorithm(test6, expected_result, best_fit, "best_fit");
